...
import com.indicorpit.tambola.databinding.ActivityGameStartHostScreenBinding;
...
import com.indicorpit.tambola.viewModel.GameStartHostViewModel;
...
public class GameStartHostScreen extends AppCompatActivity {
    ...
    ValueEventListener claimsValueEventListener;
    ValueEventListener gameOverValueEventListener;
    int tempCounter=0;
    ...
    void initializeScreen()
    {
        ...
        binding.gameOverButton.setOnClickListener ( view ->
                new AlertDialog.Builder(GameStartHostScreen.this)
                        .setTitle("Are you sure to Game Over")
                        .setNegativeButton ("Cancel",null  )
                        .setPositiveButton ( "Yes" , ( dialogInterface , i ) -> setGameOver () )
                        .show () );
        ...
        if(gameStartHostViewModel.getRoomCode ()==null)
        {
            ...
            childEventListener1= Database.getDatabaseReference ().child ( "Room" ).child( gameStartHostViewModel.getRoomCode ( ) ).child ( "players" ).addChildEventListener ( new ChildEventListener ( ) {
                @Override
                public void onChildAdded ( @NonNull DataSnapshot snapshot , @Nullable String previousChildName ) {
                    ...
                    if(playerId!=null)
                    {
                        ...
                        childEventListener2= Database.getDatabaseReference ().child ( "Room" ).child( gameStartHostViewModel.getRoomCode ( ) ).child ( "players" ).child ( playerId )
                           ...
                           claimsValueEventListener=Database.getDatabaseReference ().child ( "Room" ).child( gameStartHostViewModel.getRoomCode ( )).child ( "players" ).child ( playerId )
                                            ...
                                            Database.getDatabaseReference ().child ( "Room" ).child( gameStartHostViewModel.getRoomCode ( )).child ( "players" ).child ( playerId )
                                                ...
                                                public void onDataChange ( @NonNull DataSnapshot snapshot ) {
                                                    ...
                                                    if(gameStartHostViewModel.getPlayerName ()!=null)
                                                    {
                                                        ...
                                                        layoutHostClaimsBinding.ticketId.setText ( gameStartHostViewModel.getClaimModelClass ( ).getTicketId ());
                                                        ...
                                                        if(gameStartHostViewModel.getClaimModelClass ().getClaimStatus ().contains ( "Pending" ))
                                                        {
                                                            ...
                                                            builder1.setPositiveButton ( "APPROVE" , ( dialogInterface , i ) -> {
                                                                ...
                                                                Database.getDatabaseReference ().child ( "Room" ).child( gameStartHostViewModel.getRoomCode ( ) ).child ( "players" ).child ( playerId )
                                                                        .child ( "tickets" ).child ( gameStartHostViewModel.getClaimModelClass ( ).getTicketId () ).child ( "claims" ).setValue ( gameStartHostViewModel.getClaimModelClass () ).addOnCompleteListener ( task -> {
                                                                    if(task.isSuccessful ())
                                                                    {
                                                            ...
                                                            builder1.setNegativeButton ( "BLOCK" , ( dialogInterface , i ) -> Database.getDatabaseReference ().child ( "Room" ).child( gameStartHostViewModel.getRoomCode ( ) ).child ( "blocks" ).child ( "playerId" ).setValue ( gameStartHostViewModel.getClaimModelClass ().getPlayerId() ).addOnCompleteListener ( task -> {
                                                                if(task.isSuccessful ())
                                                                {
                                                                    ...
                                                                    Database.getDatabaseReference ().child ( "Room" ).child( gameStartHostViewModel.getRoomCode ( ) ).child ( "players" ).child ( playerId )
                                                                            .child ( "tickets" ).child ( gameStartHostViewModel.getClaimModelClass ( ).getTicketId () ).child ( "claims" ).setValue ( gameStartHostViewModel.getClaimModelClass () ).addOnCompleteListener ( task12 -> {
                                                                        if( task12.isSuccessful ())
                                                                        {
                                                                            ...
                                                                            Database.getDatabaseReference ().child ( "Room" ).child(gameStartHostViewModel.getRoomCode ( )).child ( "blocks" ).child("tickets").child ( gameStartHostViewModel.getClaimModelClass ( ).getTicketId () ).setValue ( "Blocked" ).addOnCompleteListener ( task121 -> {
            ...
            gameOverValueEventListener= Database.getDatabaseReference ().child ( "Room" ).child(gameStartHostViewModel.getRoomCode ()).child("gameOver").addValueEventListener ( new ValueEventListener ( ) {
                @Override
                public void onDataChange ( @NonNull DataSnapshot snapshot ) {
            ...
            Database.getDatabaseReference ().child ( "Room" ).child(gameStartHostViewModel.getRoomCode ()).child ( "current_number" )
                    .setValue ( gameStartHostViewModel.getCurrentNumberList ().get ( gameStartHostViewModel.getCurrentNumberList ().size ()-1 ) ).addOnCompleteListener ( task -> {
                        if(task.isSuccessful ())
                        {
                            ...
                            {
                                new Handler (  ).postDelayed ( this::setGameOver , 10000 );
                            }else
                            {
                                generateRandomNumber ( gameStartHostViewModel.getCurrentNumberList () );
    ...
    public void onBackPressed () {
        new AlertDialog.Builder(GameStartHostScreen.this)
                .setMessage("Are you sure to Close Room?")
                .setPositiveButton(android.R.string.yes, ( dialog , which ) -> {
    ...
    protected void onDestroy () {
        ...
        if(claimsValueEventListener!=null)
        Database.getDatabaseReference ().removeEventListener ( claimsValueEventListener );
        if(gameOverValueEventListener!=null)
        Database.getDatabaseReference ().removeEventListener (  gameOverValueEventListener);

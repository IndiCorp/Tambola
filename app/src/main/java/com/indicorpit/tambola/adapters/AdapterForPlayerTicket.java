...
import com.indicorpit.tambola.GameStartPlayerScreen;
import com.indicorpit.tambola.R;
import com.indicorpit.tambola.database.Database;
import com.indicorpit.tambola.databinding.CustomRawForPlayersTicketBinding;
...
public class AdapterForPlayerTicket extends RecyclerView.Adapter {
    ...
    public AdapterForPlayerTicket ( GameStartPlayerViewModel gameStartPlayerViewModel) {
        this.selectedIntegerList = gameStartPlayerViewModel.getSelectedIntegerList ();
        this.allIntegerList=gameStartPlayerViewModel.getAllIntegerList ();
        this.ticketList=gameStartPlayerViewModel.getTicketList ();
        this.roomId=gameStartPlayerViewModel.getRoomCode ( ) ;
        this.playerId= gameStartPlayerViewModel.getPlayerId ( ) ;
        this.ticketListId=  gameStartPlayerViewModel.getTicketListId ();
        this.pricesRoomModel=gameStartPlayerViewModel.getPricesRoomModel ( );
        this.hashMap= gameStartPlayerViewModel.getHashMap ( );
        this.gameStartPlayerViewModel=gameStartPlayerViewModel;
    }
    ...
    public void onBindViewHolder ( @NonNull RecyclerView.ViewHolder holder , int position ) {
       ...
       GameStartPlayerScreen.childEventListenerAdapter= Database.getDatabaseReference ().child ( "Room" ).child(roomId).child ( "claims" ).addChildEventListener ( new ChildEventListener ( ) {
            @Override
        ...
        GameStartPlayerScreen.valueEventListenerAdapter= Database.getDatabaseReference ().child ( "Room" ).child(roomId).child ( "blocks" ).child("players").addValueEventListener ( new ValueEventListener ( ) {
            @Override
        ...
        GameStartPlayerScreen.childEventListenerAdapter2= Database.getDatabaseReference ().child ( "Room" ).child(roomId).child ( "blocks" ).child("tickets").addChildEventListener ( new ChildEventListener ( ) {
            @Override
            public void onChildAdded ( @NonNull DataSnapshot snapshot , @Nullable String previousChildName ) {
    ...
    void claimedClick( View view,MyHolder myHolder) {
        ...
        }else if(gameStartPlayerViewModel.isClaimPending ())
        {
            showAlertOnEmptyClaim(myHolder,"Claim Decision Pending By Host");
        }else
        {
            if(selectedIntegerList.get ( myHolder.getAdapterPosition () ).size ()>0)
            {
                if(button.getId ()==myHolder.binding.firstLineClaimButton.getId ())
                      firstLineClaimButtonClicked ( myHolder );
    ...
    void firstLastClaimButtonClaimClicked(MyHolder myHolder) {
        new AlertDialog.Builder(myHolder.binding.getRoot ().getContext ())
                ...
                .setPositiveButton("Yes", ( dialog , which ) -> {
                    ...
                    ClaimModelClass claimModelClass=new ClaimModelClass ( integerListToString (myHolder.getAdapterPosition ()),claimType ,ticketList.get ( myHolder.getAdapterPosition ( ) ),claimStatus,playerId,ticketListId.get ( myHolder.getAdapterPosition ( ) ));
                    ...
                    Database.getDatabaseReference ().child ( "Room" ).child(roomId).child ( "players" ).child ( playerId )
                            ...
                            .addOnCompleteListener ( task -> {
                                if( task.isSuccessful ( ) )
                                {
                                    ...
                                    gameStartPlayerViewModel.setClaimPending ( true );
                                }
                                else
    ...
    void firstLineClaimButtonClicked(MyHolder myHolder) {
        new AlertDialog.Builder(myHolder.binding.getRoot ().getContext ())
                .setTitle("Are you sure to Claim")
    ...
    boolean checkClaim(int position,String claimType) {
        ...
        switch (claimType) {
            ...
            case  "Early Five":
                if(selectedIntegerList.get ( position ).size ()<5)
                {
                ...
                {
                    int count=0;
                    for (int num : selectedIntegerList.get ( position )) {
